...
public class ItemsManager : MonoBehaviour
{
    ...
    public void InstantiateSelections()
    {
        ...
        for (int i = 0; i < itemParents.Length; i++)
        {
            ...
            Debug.Log(itemParents[i].name);
            if (itemParents[i].transform.childCount == 0)
            {
                Debug.Log("instantiating at: " + itemParents[i].name);
                // GameObject itemObj = Instantiate(newChoice, itemParents[i].transform.position, Quaternion.identity);
                // itemObj.transform.SetParent(itemParents[i].transform, true);
            ...
            Debug.Log("going to remove from list now");
